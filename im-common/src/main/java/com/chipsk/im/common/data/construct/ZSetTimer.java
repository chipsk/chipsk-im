...
import com.chipsk.im.common.util.SerializeUtil;
import com.chipsk.im.common.util.SerializeUtils;
 ...
 * 由于任务自身有{@link com.chipsk.im.common.exception.ExceptionHandler}，所以客户端可自定义实现任务失败后重发到redis等这样逻辑的Handler
 *
 * @author chengpan
 */
...
//    private String queueName = "delay_task";

    private Jedis jedis = null;



    private Set<TimerTask> unprocessedTasks = new HashSet<>();

    private Thread workerThread = new Thread(new Worker());

    public void doAddTask(TimerTask job, int delay, TimeUnit unit) {
        long score = System.currentTimeMillis() + unit.toMillis(delay);
        jedis.zadd(queueName, score, SerializeUtils.serialize(job));
//        jedis.zadd(queueName, score, JSON.toJSONBytes(job));
     ...
     */
    public Collection<TimerTask> onStop() {
        jedis.close();
    ...
    private class Worker implements Runnable {
        ...
        public void run() {
            ...
            while (WORKER_STATE_UPDATER.get(ZSetTimer.this) == WORKER_STATE_STARTED) {
                ...
                List<TimerTask> tasks = takeExpireTask();
        ...
        private List<TimerTask> takeExpireTask() {
            Set<byte[]> set = getExpiredTaskData();
            return serializeToTask(set);
        }

        private List<TimerTask> serializeToTask(Set<byte[]> set) {
            if (set == null)
                return Collections.emptyList();
            List<TimerTask> list = new ArrayList<>(set.size());
            set.forEach((b) -> list.add((TimerTask) SerializeUtils.deserialize(b)));
//            set.forEach((b) -> list.add(JSON.parseObject(b, TimerTask.class)));
            return list;
        }
